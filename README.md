...
| `lps api push` | `api/`, `lib/` | Custom REST routes: a plain `GET` with the default `manage_options` permission (`hello.php`), a public `permission()` override with CORS `headers()` for a headless caller (`newsletter-signup.php`), one (`prices-in-currency.php`) enriching the `price` ACF field from `acf/field-groups/group_demo_pricing.json` with a live rate from an external currency API through `guzzlehttp/guzzle`. `post-pdf/[post_id].php` shows a dynamic path segment rendering any published post as a downloadable PDF with `dompdf/dompdf` (fed the post's own `the_content`-filtered HTML), raw binary output instead of the usual JSON (headers set and echoed directly, since WordPress always JSON-encodes a normal route return value), mirroring the [post-to-PDF cookbook recipe](https://docs.loopress.dev/cookbook/documents-and-files/post-to-pdf-dompdf-wordpress-rest-api/) line for line, post 1 ("Hello world!", WordPress's own default) is real seeded content here, no stand-in needed. `orders-export.php` turns the same priced pages into a downloadable `.xlsx` with `phpoffice/phpspreadsheet`, standing in for the [WooCommerce-orders cookbook recipe](https://docs.loopress.dev/cookbook/documents-and-files/spreadsheet-export-phpspreadsheet-wordpress-rest-api/) it mirrors without pulling WooCommerce into this demo. `webhook.php` uses `#[Permission]` for per-verb authorization (a public health-check `get()`, a signed `post()`). `orders/[order_id]/items/[item_id].php` combines two dynamic segments with a `#[Permission]` callback shared from `lib/ApiKeyGuard.php`. `WITH_MAJ_ENDPOINT.php` is deliberately invalid (uppercase filename), meant to demonstrate the client-side filename check rejecting it before any network call. `graphql.php` answers a `POST` GraphQL query through `webonyx/graphql-php`, mirroring the [GraphQL-route cookbook recipe](https://docs.loopress.dev/cookbook/search-and-data-services/graphql-wordpress-rest-api-route/) line for line: it resolves against WooCommerce's real `product` post type and `_price` meta, which this demo doesn't install, so the query still executes and returns a well-formed `product: null` here. No page in this demo actually sets a price either, so `prices-in-currency.php`'s response and `orders-export.php`'s spreadsheet are both well-formed but empty too, the point of all three is the external call and the Composer/API pairing, not seeded content. `oauth/authorize.php`, `oauth/token.php`, and `oauth/me.php`, backed by the shared plumbing in `lib/Oauth/*.php`, mirror the [OAuth2 PKCE login cookbook recipe](https://docs.loopress.dev/cookbook/auth-and-security/oauth2-pkce-login-wordpress-rest-api/) line for line: Authorization Code plus PKCE on `league/oauth2-server`, so a frontend-only app could log a user in against this site's own accounts. No client is registered against this instance (that's a one-off `OauthClients::register()` call, see the cookbook recipe's "Registering a client"), so the three routes are well-formed but unreachable end-to-end here, same caveat as the other Composer/API pairings above. |
...
| `lps composer push` | `composer.json` | Installs `hello-dolly` from wpackagist (a WordPress plugin, visible under `wp-content/plugins/` afterward), `guzzlehttp/guzzle` (`use`d by `api/prices-in-currency.php`), `dompdf/dompdf` (`use`d by `api/post-pdf/[post_id].php`), `phpoffice/phpspreadsheet` (`use`d by `api/orders-export.php`), `webonyx/graphql-php` (`use`d by `api/graphql.php`), and `league/oauth2-server` plus its `nyholm/psr7`/`nyholm/psr7-server` PSR-7 bridge (`use`d by `api/oauth/*.php` and `lib/Oauth/*.php`). |
