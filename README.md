...
| `lps composer push` | `composer.json` | Installs `hello-dolly` from wpackagist (a WordPress plugin, visible under `wp-content/plugins/` afterward), `guzzlehttp/guzzle` (`use`d by `api/prices-in-currency.php`), `dompdf/dompdf` (`use`d by `api/post-pdf/[post_id].php`), and `phpoffice/phpspreadsheet` (`use`d by `api/orders-export.php`). |
